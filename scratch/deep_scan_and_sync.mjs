...
let cursor = "";
let page = 1;
const maxPages = 15; // 15 pages * 12 = 180 recent posts
let totalChecked = 0;
let added = 0;
let updated = 0;

console.log(`Starting deep scan across posts of @${username}...`);
...
while (page <= maxPages) {
  ...
  for (const edge of edges) {
    ...
    if (hasShitet || hasEpisod) {
      ...
      if (existing) {
        ...
        updated++;
        console.log(`  MATCH (UPDATE): [${category}] [${status}] https://instagram.com/p/${node.shortcode}/ - "${caption.slice(0, 50)}..."`);
      } else {
        ...
        added++;
        console.log(`  MATCH (ADD): [${category}] [${status}] https://instagram.com/p/${node.shortcode}/ - "${caption.slice(0, 50)}..."`);
      }
    }
  }
  ...
  if (!pageInfo?.has_next_page || !pageInfo?.end_cursor) {
    console.log("Reached end of profile timeline.");
    break;
  }
...
console.log(`\nScan finished: checked ${totalChecked} total posts.`);
